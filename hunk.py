...
from libra.colors import debug, info, warn, error, fail, e, Style, Fore
...
class ActiveScanner:
    ...
    def run_cmd(self, cmd, tag='?', redirect=None):
        ...
        info(('Skipping' if self.dryrun else 'Running') + ' task {bgreen}{tag}{rst}' + (
            ' with {bblue}{cmd}{rst}' if self.verbose >= 1 else '...'))

        if self.dryrun:
            return True

        proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE if redirect else subprocess.DEVNULL,
        ...
        for service in nmap_svcs:
            ...
            info(
                'Service {bgreen}{service.port}{rst}/{bgreen}{service.protocol}{rst} is {bgreen}{service.service}{rst}' + (
                    ' running {green}' + service.service_dict[
                        'product'] + '{crst}' if 'product' in service.service_dict else '') + (
                    ' version {green}' + service.service_dict[
                        'version'] + '{crst}' if 'version' in service.service_dict else ''))
            services.append(
                (address, service.port * -1 if service.protocol == 'udp' else service.port, service.service))
    ...
    def run_amap(self, services, only_unidentified=True):
        ...
        for i, val in enumerate(services):
            for amap_svc in amap_svcs:
                if services[i][0] == amap_svc[0] and services[i][1] == amap_svc[1] and (
                        'unknown' in services[i][2] or not only_unidentified):
                    services[i] = amap_svc
    ...
    def enum_dns(self, address, port, service, basedir):
        ...
        info('Running task {bgreen}nmblookup-{port}{rst}' + (
            ' with {bblue}' + nmblookup + '{rst}' if self.verbose >= 1 else '...'))
    ...
    def scan_service(self, address, port, service):
        ...
        info('Scanning service {bgreen}{service}{rst} on port {bgreen}{port}{rst}/{bgreen}{proto}{rst}...',
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Network reconnaissance tool for enumerating the everliving fuck out of a host.')
    parser.add_argument('address', action='store', help='address of the host.')
    parser.add_argument('port', action='store', type=int, help='port of the service, if scanning only one port',
                        nargs='?')
    ...
    parser.add_argument('-s', '--deep-scan', action='store_true',
                        help='re-scans each service separately with broader settings')
    ...
    parser.add_argument('--nmap', action='store', default='-Pn --min-rate=400 -T4 --script-timeout 10m',
                        help='additional nmap arguments')
    parser.add_argument('--hydra', action='store', default='-L data/users -P data/passwords -t 16 -f',
                        help='additional hydra arguments')
